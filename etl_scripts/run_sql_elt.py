...
database_cursor.execute(
    """
    DROP TABLE IF EXISTS funnel_throughput;
    """
)

database_cursor.execute(
    """
    -- Create the funnel_throughput table
    CREATE TABLE IF NOT EXISTS funnel_throughput (
...
database_cursor.execute(
    """
    INSERT INTO funnel_throughput
    SELECT '1 - Applications' AS funnel_stage, COUNT(DISTINCT(application_id)) AS throughput FROM staging_applications
    UNION ALL
    SELECT '2 - Response Offers' AS funnel_stage, COUNT(DISTINCT(application_id)) as throughput FROM staging_responses WHERE response_type = 'loan_offer'
    UNION ALL
    SELECT '3 - Acceptances' AS funnel_stage, COUNT(DISTINCT(application_id)) as throughput FROM staging_responses WHERE response_type = 'loan_offer' AND accepted_by_customer = 1
    UNION ALL
    SELECT '4 - Payouts' AS funnel_stage, COUNT(DISTINCT(application_id)) as throughput FROM staging_applications WHERE paid_out_date IS NOT NULL;
    """
)
